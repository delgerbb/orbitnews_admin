...
if ((isset($_POST['action']) ? $_POST['action'] : (isset($_GET['action']) ? $_GET['action'] : '')) == 'simple') {
    ...
    if ($handle->uploaded) {
		...
		$handle->file_new_name_body = $newImageFileName;
		$handle->file_name_body_add = '_X'.$smallest_X;
		
		if($orginImage_X_pixels > $smallest_X){
			$handle->image_x                 = $smallest_X;
		}else{
			$handle->image_x                 = $orginImage_X_pixels;
		}
		
		//echo("this is width before image in process: ". $handle->image_src_x);
		
		//$handle->image_watermark = 'gobi_logo_new.png';

		//$handle->image_watermark_position = 'BR';
		//$handle->file_new_name_body = 'delger';
        ...
        $handle->image_resize            = true;
        $handle->image_ratio_y           = true;
        //$handle->image_x                 = $smaller_X;
		$handle->file_new_name_body = $newImageFileName;
		$handle->file_name_body_add = '_X'.$smaller_X;
		
		if($orginImage_X_pixels > $smaller_X){
			$handle->image_x                 = $smaller_X;
		}else{
			$handle->image_x                 = $orginImage_X_pixels;
		}
		...
		$handle->file_new_name_body = $newImageFileName;
		$handle->file_name_body_add = '_X'.$small_X;
		
		if($orginImage_X_pixels > $small_X){
			$handle->image_x                 = $small_X;
		}else{
			$handle->image_x                 = $orginImage_X_pixels;
		}
        ...
        $handle->image_resize            = true;
        $handle->image_ratio_y           = true;
        //$handle->image_x                 = $large_X;
		$handle->file_new_name_body = $newImageFileName;
		$handle->file_name_body_add = '_X'.$large_X;
		
		if($orginImage_X_pixels > $large_X){
			$handle->image_x                 = $large_X;
		}else{
			$handle->image_x                 = $orginImage_X_pixels;
		}
        ...
        //$handle->image_ratio_y           = true;
        //$handle->image_x                 = 1050;
		//$handle->file_name_body_add = '_X1050';
		$handle->file_new_name_body = $newImageFileName;
		
		//if($orginImage_X_pixels > 1050){
			//$handle->image_x                 = 1050;
		//}else{
			//$handle->image_x                 = $orginImage_X_pixels;
		//}
